...
function Navbar() {
  ...
  useEffect(() => {
    const getSearchHistory = async () => {
      const res = await axios.get(`${backendURL}/get-history`);
  ...
  const saveHistory = async () => {
    try{
      const res = await axios.post(`${backendURL}/add-search-history/${email}`,{search: searchedData})
      ...
      <datalist id="searchHistory">
          {
            searchHistory?.map(his => (
              <option value={his?.search}>{his?.search}</option>
            ))
          }
      </datalist>
    </>
